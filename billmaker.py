...
class AstaEpsilonBilling(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('Asta Epsilon Infotronics - Billing System v1.1')
        ...
        self.btn_docx.setStyleSheet("padding: 8px;")
    ...
    def clear_form(self):
        ...
        QMessageBox.information(self, "Form Cleared", "The form has been reset.")
    ...
    def process_bill(self, export_pdf):
        ...
        for i in range(3):
            # Safe data extraction from UI table
            ...
            try:
                # Calculations for the amount placeholder
                qty = float(qty_val)
                price = float(price_val)
                line_total = qty * price
                grand_total += line_total
                
                # Mapping to your Word template placeholders
                data[f"{{{{description{i+1}}}}}"] = desc
                data[f"{{{{quantity{i+1}}}}}"] = str(int(qty) if qty.is_integer() else qty)
                data[f"{{{{amount{i+1}}}}}"] = f"{line_total:,.2f}"
            except ValueError:
                # Handle empty rows or non-numeric input gracefully
                data[f"{{{{description{i+1}}}}}"] = desc
                data[f"{{{{quantity{i+1}}}}}"] = ""
                data[f"{{{{amount{i+1}}}}}"] = ""
    ...
    def save_document(self, data, export_pdf):
        ...
        if not os.path.exists(template_path):
            QMessageBox.critical(self, "Error", f"Template file '{template_name}' not found.")
            return
        ...
        if output_path:
            try:
                ...
                # Gather all text areas (paragraphs and table cells)
                ...
                # Replace placeholders while preserving font styles
                for p in all_paras:
                ...
                if export_pdf:
                    # Convert to PDF and cleanup temp docx
                    convert(temp_docx, output_path)
                    if os.path.exists(temp_docx):
                        os.remove(temp_docx)
                else:
                    # For docx, just rename the temp file to the output name
